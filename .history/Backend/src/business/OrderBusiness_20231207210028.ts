  ...
  export class OrderBusiness {
    ...
    public createOrder = async (
      ...
      const items: IOrderItem[] = itemsInput.map((item) => {
        ...
        return { ...item, price: 0 }; // Inicializa o preço como 0, será atualizado abaixo
      });
  
      // Para cada item, obtenha o preço do banco de dados e atualize a propriedade 'price'
      for (let item of items) {
        const price = await this.orderDatabase.getPrice(item.name);
    ...
    public getOrders = async (): Promise<IGetOrdersOutputDTO> => {
      ...
      for (let orderDB of ordersDB) {
        ...
        order.setOrderItens(orderItemsDB);
        orders.push(order);
      }
      ...
      const response: IGetOrdersOutputDTO = {
        orders: orders.map((order) => ({
          ...
          items: order
            ...
            .map((item) => ({
              name: item.item_name,
              type: item.item_type, // Adicione o tipo do item
              quantity: item.quantity,
              price: item.price,
            })),
