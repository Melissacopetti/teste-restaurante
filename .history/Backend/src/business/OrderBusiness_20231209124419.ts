  ...
  public createOrder = async (
    ...
    const items: IOrderItem[] = itemsInput.map((item) => {
      ...
      return {
        ...
        price: 0,
        order_id: this.dataConversionService.convertOrderId(item.order_id), // Use the actual conversion method
      };
    });
    ...
    const items: IOrderItem[] = itemsInput.map((item) => {
      ...
      return {
        ...
        quantity: item.quantity,
        price: 0,
        order_id: this.dataConversionService(item.order_id), // Use the converted order ID
      };
    });
