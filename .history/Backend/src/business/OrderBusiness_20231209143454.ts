...
export class OrderBusiness {
  ...
  const orderBusiness = new OrderBusiness(orderDatabase, idGenerator, dataConversionService);
  ...
  public getOrders = async (): Promise<IGetOrdersOutputDTO> => {
    const ordersDB: IOrderDB[] = await this.orderDatabase.getOrders();

    const orders: Order[] = [];

    for (let orderDB of ordersDB) {
        const order = new Order(parseInt(orderDB.id), []); // Convertemos o ID para número
        const orderItemsDB: IOrderItemDB[] = await this.orderDatabase.getOrderItem(orderDB.id.toString());

        for (let orderItemDB of orderItemsDB) {
            const price = await this.orderDatabase.getPrice(orderItemDB.item_name);
            orderItemDB.price = price || 0; // Definimos um valor padrão caso o preço seja undefined
        }
    ...
    const response: IGetOrdersOutputDTO = {
        orders: orders.map((order) => ({
            id: order.getId().toString(), // Convertemos o ID para string
            items: order
                .getOrderItems()
                .map((item) => ({
                    name: item.item_name,
                    ...
                    quantity: item.quantity,
                    price: item.price,
                    order_id: item.order_id.toString(), // Convertemos o order_id para string
                })),
